...
import { simulateRealTimeUpdates } from '@/utils/realTimeUpdates';

export default function SearchPage() {
  const { setSelectedSpot, setCurrentView } = useParking();
  ...
  const { data: parkingSpots = [], isLoading } = useQuery({
    ...
    select: (data: ParkingSpot[]) => data,
  });

  // Initialize real-time updates
  useEffect(() => {
    if (parkingSpots.length > 0) {
      setRealTimeSpots(parkingSpots);
      const interval = simulateRealTimeUpdates(parkingSpots, setRealTimeSpots);
      return () => clearInterval(interval);
    }
  }, [parkingSpots]);

  // Filter spots based on current filters
  const filteredSpots = realTimeSpots.filter(spot => {
    ...
    const matchesPrice = priceFilter === 'all' || (() => {
      ...
      switch (priceFilter) {
        case '20-50': return rate >= 20 && rate <= 50;
    ...
    const matchesAvailability = availabilityFilter === 'all' || 
      spot.status === availabilityFilter;
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">Loading parking spots...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-muted-foreground">
            <span data-testid="text-total-spots">{filteredSpots.length}</span> parking spots found in Sikkim
            <span className="ml-4 text-secondary">
              <div className="inline-block w-2 h-2 bg-secondary rounded-full mr-1" />
              <span data-testid="text-available-spots">{availableCount}</span> available now
            </span>
          </p>
        </div>
